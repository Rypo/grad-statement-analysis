"""Adapted from fast.ai's text module"""
import re
import html
import pickle
import collections
from types import SimpleNamespace
from typing import Any, AnyStr, Callable, Collection, List
from concurrent.futures import ProcessPoolExecutor
import spacy
from spacy.symbols import ORTH


def ifnone(test_none, else_case):
    return test_none if test_none is not None else else_case
...
defaults.cpus = 1
defaults.text_spec_tok = [UNK,PAD,BOS,EOS,FLD,TK_MAJ,TK_UP,TK_REP,TK_WREP] #+ [TK_QUOB,TK_QUOE,TK_DEL,TK_RED,TK_BLU,TK_GRN,TK_BLD,TK_ITL,TK_HLB,TK_HLE]


class BaseTokenizer():
    "Basic class for a tokenizer function."
    def __init__(self, lang:str): self.lang = lang
    def tokenizer(self, t:str): return t.split(' ')
...
class Tokenizer():
    ...
    def __init__(self, tok_func:Callable=SpacyTokenizer, lang:str='en', pre_rules=None,
                 post_rules=None, special_cases=None, n_cpus:int=None):
    ...
    def process_all(self, texts) -> List[List[str]]:
...
class Vocab():
    ...
    @classmethod
    def create(cls, tokens, max_vocab:int, min_freq:int) -> 'Vocab':
...
__all__ = ['BaseTokenizer', 'SpacyTokenizer', 'Tokenizer', 'Vocab', 'fix_html', 'replace_all_caps', 'replace_rep', 'replace_wrep',
           ...
           'deal_caps'] #+ ['TK_QUOB','TK_QUOE','TK_DEL','TK_RED','TK_BLU','TK_GRN','TK_BLD','TK_ITL','TK_HLB','TK_HLE']